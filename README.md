...
- **6 contraintes Databricks Free Edition serverless rencontrées et documentées** :
  ...
  5. `CLEAR CACHE` (`spark.catalog.clearCache()`) bloqué serverless → wrap try/except dans S5 Partie B
  6. `PERSIST TABLE` (`.cache()` / `.persist()`) bloqué serverless → Optim 1 cache non testable, gain attendu théorique documenté à la place
